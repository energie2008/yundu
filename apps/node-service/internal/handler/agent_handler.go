...
func (h *AgentHandler) Heartbeat(c *gin.Context) {
	...
	if rt != nil && h.deploymentService != nil {
		...
		if err != nil {
			...
			// D-自愈：三通道统一不变式检查（版本一致+运行中→applied；其余超 5 分钟→pushed 重拉）。
			// 复用心跳周期（10s）作巡检触发点，零额外 goroutine（根因 #4 根治）。
			kernelRunning := req.RuntimeStatus != "inactive"
			h.deploymentService.SelfHealDispatchStatus(c.Request.Context(), rt.ID, targetVersion.VersionNo, kernelRunning)
